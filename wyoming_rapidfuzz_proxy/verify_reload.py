...
async def test_hot_reload():
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # 2. Start Manager
        manager = SentenceManager(tmp_path, lang, "ws://localhost", "token", poll_interval=1.0)
        await manager.start()
        ...
        last_hash = manager._file_hash
        yaml_path.touch()
        await asyncio.sleep(1.5)
        
        # Verify hash didn't change (internal check, but we can check if manager reloaded by logging or mocking, 
        # here we just check if hash is same which implies no reload logic triggered if we trust the code)
        assert manager._file_hash == last_hash
